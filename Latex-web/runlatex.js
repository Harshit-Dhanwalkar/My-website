...
runlatex.texts = {
  ...
  "TeXLive.net": "run at TeXLive.net", // or "run latex"
...
runlatex.latexcgiURI = "https://texlive.net/cgi-bin/latexcgi2";
...
runlatex.packageregex = [
  ...
  [
    /\\begin{equation|align|gather|flalign|\\DeclareMathOperator/,
    "\\usepackage{amsmath}\n",
  ],
...
const commentregex = / %.*/;
const engineregex =
  /% *!TEX.*[^a-zA-Z](((pdf|xe|lua|u?p)?latex(-dev)?)|asy|context|(pdf|xe|lua|[ou]?p)?tex) *\n/i;
const returnregex =
  /% *!TEX.*[^a-zA-Z](ngpdf|pdfjs|pdf|log|make4ht|latexml|lwarp) *\n/i;
    ...
    if (!pretext.match(noeditregex) && !p[i].classList.contains("noedit")) {
      if (
        (runlatex.adddefaultpreamble &&
          (pretext.match(norunregex) ||
            (pretext.match(/\n[^\n]/g) || "").length + 1 <
              runlatex.minrunlines)) ||
        (!runlatex.adddefaultpreamble &&
          pretext.indexOf("\\documentclass") == -1 &&
          !pretext.match(engineregex)) ||
        ...
        if (runlatex.overleafURI) {
          ...
          f.innerHTML =
            '<form style="display:none" id="form-pre' +
            i +
            '" action="' +
      ...
      for (prop in incl) {
        if (editors[prop] == null) {
          addinput(
            fm,
            "encoded_snip[]",
            document.getElementById(prop).textContent,
          );
        } else {
...
function generatepreamble(t, e) {
  ...
  for (var i = 0; i < runlatex.packageregex.length; i++) {
    if (t.match(runlatex.packageregex[i][0]))
      e.insert(runlatex.packageregex[i][1]);
  }
...
if (rldefaultreturn == "") rldefaultreturn = "ngpdf";
